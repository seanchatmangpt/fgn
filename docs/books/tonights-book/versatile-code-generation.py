...
class Node:
    def __init__(self, coordinates):
        self.coordinates = np.array(
            coordinates
        )  # Convert coordinates to numpy array for easier computation
...
class PKERRT:
    def __init__(self, start, goal, obstacle_list, size):
        ...
        self.tree = PathFindingTree(
            self.start, self.goal
        )  # Initialise the path as a PathFindingTree object
    ...
    def plan(self):
        while True:
            ...
            nearest_node = self.get_nearest_node(
                random_node
            )  # Get nearest Node from the random Node

            new_node = self.move_towards(
                nearest_node, random_node
            )  # Get new Node by moving from nearest Node to random Node

            if self.check_collision(
                new_node
            ):  # If there is a collision, continue to next iteration
                continue

            self.tree.add_node(
                new_node
            )  # If no collision, add the new Node to the tree

            if (
                distance.euclidean(new_node.coordinates, self.goal) < 1.0
            ):  # If the new Node is close to the goal, return the entire path
                return self.get_path(new_node)

    def get_random_node(self):
        return Node(
            np.array([random.uniform(0, self.size), random.uniform(0, self.size)])
        )  # Return a random Node within our 2D grid

    def get_nearest_node(self, node):
        distances = [
            distance.euclidean(n.coordinates, node.coordinates) for n in self.tree.nodes
        ]  # Calculate distance of 'node' from all nodes in our tree path
    ...
    def move_towards(self, nearest_node, random_node):
        direction = (
            ...
            random_node.coordinates - nearest_node.coordinates
        )  # Calculate the direction from nearest Node to random Node
        new_node = Node(
            nearest_node.coordinates + direction
        )  # Create new Node in this direction
    ...
    def check_collision(self, node):
        for ob in self.obstacle_list:  # Loop through all obstacles
            if (
                distance.euclidean(node.coordinates, ob) < 1.0
            ):  # If Node is within 1.0 distance of any obstacle, return True (collision)
                return True
    ...
    def get_path(self, node):
        ...
        while (
            node is not None
        ):  # Start from goal and go to start by going to parent at each step
...
class TestPkerrt(unittest.TestCase):
    def setUp(self):
        ...
        )  # Initialize pkerrt with start, end, obstacles and grid size = 10
    ...
    def test_get_random_node(self):
        ...
        self.assertTrue(
            ...
            and 0
            <= node.coordinates[1]
            <= self.pkerrt.size  # Test that its coordinates are within the grid size
        )
