...
parser = argparse.ArgumentParser(description="Run an AGI agent system for ubiquitous language.")
...
parser.add_argument("--requirements", type=str, default=default_requirements,
...
def generate_term(term_type, schema):
    ...
    system_description = schema.get('classes', {}).get('UbiquitousLanguage', {}).get('description', '')
    ...
    current_terms = schema.get('classes', {}).get('UbiquitousLanguage', {}).get('slots', {}).get(term_type, [])

    # Construct the prompt using the specialized prompt and the current terms to avoid
    prompt = specialized_prompts[term_type] + (
        f" Avoid the following existing terms: {', '.join(current_terms)}" if current_terms else "")
...
def closed_loop_agent_system(num_iterations: int = args.iterations, delay: int = args.delay) -> None:
    schema = load_schema()
    if not schema:
        schema = {
            "classes": {
    ...
    for _ in range(num_iterations):
        ...
        with open("discussion_log.txt", "a") as file:
            file.write(f"Discussion Iteration Completed. Updated schema saved to 'bounded_context.yaml'\n")
...
def validate_schema(schema: dict, required_completeness: float = 0.8) -> bool:
    ...
    requirements = schema.get("classes", {}).get("UbiquitousLanguage", {}).get("description", "")
    glossary = schema.get("classes", {}).get("UbiquitousLanguage", {}).get("slots", {}).get("glossary", [])
    nouns = schema.get("classes", {}).get("UbiquitousLanguage", {}).get("slots", {}).get("nouns", [])
    verbs = schema.get("classes", {}).get("UbiquitousLanguage", {}).get("slots", {}).get("verbs", [])
    ...
    # Make the OpenAI API call
    response = openai.Completion.create(engine="gpt-3.5-turbo-instruct-0914", prompt=prompt, max_tokens=10,
                                        temperature=0)
