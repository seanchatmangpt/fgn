...
def run_fgn_command(input_file, output_file, mode, title):
    try:
        if input_file:
            subprocess.run(['fgn', '-e', 'blackboard.md', '-m3', '-i', input_file, '-o', output_file, mode, title], check=True)
        else:
            subprocess.run(['fgn', '-e', 'blackboard.md', '-m3', '-o', output_file, mode, title], check=True)
